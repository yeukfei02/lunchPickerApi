...
export const getRestaurantByPhone = async () => {
  const result = await axios.get(
    `${ROOT_URL}/restaurant/find-restaurant-by-phone`,
    {
      params: {
        phone: '+85227881226'
      },
...
export const getRestaurantDetailsById = async () => {
  const result = await axios.get(
    `${ROOT_URL}/restaurant/get-restaurant-details/X7b2izv3qhklnCDjoF37tA`,
    {
      headers: {
        "Content-Type": "application/json"
      }
...
export const getRestaurantDetailsReviewById = async () => {
  const result = await axios.get(
    `${ROOT_URL}/restaurant/get-restaurant-details-review/X7b2izv3qhklnCDjoF37tA`,
    {
      headers: {
        "Content-Type": "application/json"
      }
